	...
	<el-row class="panel">
		<el-col :span="24" class="panel-top">
			<el-col :span="22" style="font-size:26px;">
				<img src="../assets/logo4.png" class="logo"> <span v-if="!selectApp" v-on:click="selectApp=!selectApp"><i style="color:#20a0ff">{{currentApp.name}}</i></span>
				<el-menu theme="dark" v-if="selectApp" default-active="CMDB" class="el-menu-demo" mode="horizontal" @select="handleSelect">
					<el-menu-item v-for="app in menuApp" :index="app.id.toString()" v-if="app.showen">{{app.name}}</el-menu-item>
				</el-menu>
			</el-col>
			<el-col :span="2">
		...
		<el-col :span="24" class="panel-center">
			...
			<aside style="width:230px;">
				...
				<!--<el-menu style="border-top: 1px solid #475669;" default-active="/table" class="el-menu-vertical-demo" @open="handleopen"-->
					<!--@close="handleclose"  theme="dark" unique-opened router>-->
                ...
                <el-menu style="border-top: 1px solid #475669;" class="el-menu-vertical-demo" @open="handleopen"
					@close="handleclose" @select="handleselect" theme="dark" unique-opened router>
                    <template v-for="(item,index) in sub_menu" v-if="item.showen">
                        <el-submenu :index="item.id.toString()">
                            <template slot="title"><i :class="item.icon"></i>{{item.name}}</template>
                            <el-menu-item v-for="child in item.sub_menu" :index="currentApp.path+child.path" v-if="child.enable">{{child.name}}</el-menu-item>
                        </el-submenu>
                    </template>
				</el-menu>
			</aside>
      ...
      onSubmit() {
        console.log('submit!');
			...
			handleSelect:function(key, keyPath) {
                var vm=this;
				vm.menuApp.forEach(function (e) {
					if(e.id.toString()===key){

						vm.currentApp=e;
						vm.sub_menu=vm.currentApp.sub_menu;
					}
				});
				this.selectApp=!this.selectApp;
			},
			//退出登录
			logout:function(){
				var _this=this;
        ...
        fetchMenuApp(){
			...
			//TODO 根据用户获取菜单
			//TODO 解决这里提示报错的问题
            vm.$http.get('/api/roles/1/menus').then(
            		(resp)=> {
				vm.menuApp = resp.data.menus;
				vm.currentApp = vm.menuApp[0];
				vm.sub_menu = vm.currentApp.sub_menu;
				}
